...
def post_new(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            post.save()
            # redirect (in following line) allows you to post or submit something only once.
...
def post_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        # the following line updates an existing form.
...
def comment_approve(request, pk):
    # mydjangosite.com/comment/2/approve === second comment gets approved.
    comment = get_object_or_404(Comment, pk=pk)
    comment.approve()
    return redirect('post_detail', pk=comment.post.pk)
